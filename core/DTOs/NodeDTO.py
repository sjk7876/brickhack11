...
class NodeDTO:
    def __init__(self, name, image, children):
        self.name = name
        self.image = image
...
def load_node(node_dict):
    ...
    for child_dict in node_dict['children']:
        node = NodeDTO(child_dict['name'], child_dict['image'], load_node(child_dict))
        children.append(node)
    return NodeDTO(node_dict['name'], node_dict['image'], children)